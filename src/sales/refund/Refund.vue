<script setup>
import refundController from "./RefundController";
import rootOptions from '@/root/RootOptions'
const cols = 12;
const sm = 4;
const md = 3;
const controller = refundController();

const model = controller.model;
const rules = controller.rules;
const change = controller.change;
rootOptions.maxWidth=1300
</script>

<template>
  <crud-form :controller="controller">
    <template #heading>Refund</template>

    <template #form-data>
      <!-- <v-col :cols="cols" :sm="sm" :md="md">
        <v-select label="Receipt" v-model="model.receipt" :rules="receiptRules" :counter="100"
          :items="$store.state.sales.receipt.mini" :loading="$store.state.sales.receipt.miniLoading"
          item-text="id" item-value="id" return-object></v-select>
      </v-col> -->
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-select-field
          label="Receipt Id"
          v-model="model.receiptId"
          :rules="rules.receiptId"
          :counter="20"
          :items="controller.receiptStore.mini"
          :headers="controller.receiptNav.menu.miniHeaders"
          @ok="controller.receiptIdOk"
        ></s-select-field>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <v-text-field
          label="Customer Name"
          v-model="model.customerName"
          disabled
        ></v-text-field>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-date-picker
          label="Refund Date"
          v-model="model.refundDate"
          :rules="rules.refundDate"
        />
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-autocomplete
          label="Refund Type"
          v-model="model.refundType"
          :rules="rules.refundType"
          :counter="100"
          :items="controller.salesStore.refundTypes"
          :loading="controller.salesStore.refundTypesLoading"
        >
        </s-autocomplete>
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-textarea
          label="Notes"
          v-model="model.notes"
          :rules="rules.notes"
          :counter="200"
          rows="1"
          auto-grow
        >
        </s-textarea>
      </v-col>
      <v-col
        :cols="cols"
        :sm="sm"
        :md="md"
        v-if="model.refundType === controller.creditMemoRefundType"
      >
        <s-text-field
          label="Credit Note Id"
          v-model="model.creditNoteId"
          :rules="controller.creditNoteIdRules"
          :counter="100"
          :disabled="controller.disableControls"
        >
        </s-text-field>
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-autocomplete
          label="Bank Account Type"
          v-model="model.bankAccountType"
          :rules="rules.bankAccountType"
          :counter="100"
          :items="controller.bankingStore.bankAccountTypes"
          :loading="controller.bankingStore.bankAccountTypesLoading"
          :disabled="controller.disableControls"
        ></s-autocomplete>
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-autocomplete
          label="Bank Account Id"
          v-model="model.bankAccountId"
          :rules="rules.bankAccountId"
          :counter="100"
          :items="controller.bankAccountStore.bankAccountsByType"
          :loading="controller.bankAccountStore.bankAccountsByTypeLoading"
          item-title="bankAccountName"
          item-value="id"

          :disabled="controller.disableControls"
        ></s-autocomplete>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Amount"
          v-model="model.amount"
          :rules="rules.amount"
          :counter="9"
          :disabled="controller.disableControls"
        ></s-number-input>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-textarea
          label="Amount Words"
          v-model="model.amountWords"
          :rules="rules.amountWords"
          :counter="200"
          rows="1"
          auto-grow
          disabled
        ></s-textarea>
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Discount"
          v-model="model.discount"
          :disabled="true"
        >
        </s-number-input>
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Withholding Tax"
          v-model="model.withholdingTax"
          :disabled="true"
        ></s-number-input>
      </v-col>
      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Amount Received"
          v-model="model.amountReceived"
          :disabled="true"
        ></s-number-input>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Amount Paid"
          v-model="model.amountPaid"
          :disabled="true"
        >
        </s-number-input>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Total Bill"
          v-model="model.totalBill"
          :disabled="true"
        >
        </s-number-input>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input label="Change" v-model="model.change" :disabled="true">
        </s-number-input>
      </v-col>

      <v-col :cols="cols" :sm="sm" :md="md">
        <s-number-input
          label="Previously Refunded"
          v-model="model.amountRefunded"
          :disabled="true"
        >
        </s-number-input>
      </v-col>
    </template>
  </crud-form>
</template>
//
<script>
// import refundModel from "./RefundModel";
// import receiptNav from '../receipt/ReceiptNav';
// import funcs from '../../utils/funcs'
// export default {
//   name: "Refund",
//   data: () => ({
//     cols: 12,
//     sm: 6,
//     md: 3,
//     maxWidth: 1000,
//     path: refundModel.path,
//     refund: refundModel.refund,
//     cancellationRefundType: 'Cancellation',
//     creditMemoRefundType: 'Credit_Memo',
//     balanceClaimRefundType: 'Balance_Claim',

//     receiptRules: [(v) => !!v || "Receipt is required", ],
//     receiptIdRules: [(v) => !!v || "Receipt Id is required",],
//     refundDateRules: [(v) => !!v || "Refund Date is required",
//     ], bankAccountTypeRules: [(v) => !!v || "Bank Account Type is required",
//     ], bankAccountIdRules: [(v) => !!v || "Bank Account Id is required",
//     ], refundTypeRules: [(v) => !!v || "Refund Type is required",
//     ], creditNoteIdRules: [(v) => !!v || "Credit Note Id is required",
//     (v) => v.length < 100 || "Credit Note Id length must be less or equal to 100",], amountRules: [(v) => !!v || "Amount is required",
//     ], notesRules: [(v) => !!v || "Notes is required",
//     (v) => v.length < 200 || "Notes length must be less or equal to 200",], amountWordsRules: [(v) => !!v || "Amount Words is required",
//     (v) => v.length < 200 || "Amount Words length must be less or equal to 200",], refundReceiptInvoicesRules: [(v) => !!v || "Refund Receipt Invoices is required",
//     ],
//   }),
//   created() {
//     this.$store.dispatch("banking/getBankAccountTypes");
//     this.$store.dispatch("sales/getRefundTypes");
//   },
//   computed: {
//     bankAccountType() {
//       return this.model.bankAccountType;
//     },

//     selectedData() {
//       return this.$store.state.search.selectedMiniItem;
//     },

//     disableControls(){
//       return this.model.refundType ===this.cancellationRefundType;
//     },
//     receiptId(){
//       return this.model.receiptId;
//     },

//     receipt(){
//       let receipt = this.$store.state.sales.receipt.obj;
//       if(receipt){
//           return receipt;
//       }
//       return this.model.receipt;
//     },

//     refundType(){
//       return this.model.refundType;
//     },
//     amount(){
//       return this.model.amount;
//     },
//     bankAccount(){
//       return this.model.bankAccount;
//     },
//     change(){
//       if(this.model.receipt.amountPaid>this.model.receipt.totalBill)
//       return Math.round(this.model.receipt.amountPaid-(this.model.receipt.totalBill+this.model.receipt.amountRefunded));
//       else return 0;
//     }
//   },

//   watch: {

//     bankAccountType() {
//       if (this.bankAccountType) {
//         this.$store.dispatch("banking/bankaccount/getBankAccountsByType", this.bankAccountType
//         );
//       }
//     },

//     bankAccount(){
//       return this.model.bankAccountId = this.bankAccount.id;
//     },

//     selectedData() {
//      if(this.selectedData){
//       this.model.receiptId = this.selectedData.id;
//      }
//     },

//     receiptId(){
//       this.receiptIdEntered();
//     },

//     receipt(){
//       let receipt = this.receipt;
//       this.model.receipt= receipt;
//       if(this.receipt){
//         this.model.customerName = receipt.customer;
//         this.setControls();

//       }
//     },
//     refundType(){

//       this.setControls();
//       if(this.refundType ===this.creditMemoRefundType){
//         this.creditNoteIdRules=[(v) => !!v || "Credit Note Id is required",];
//       }

//     },
//     amount(){
//        this.model.amountWords= funcs.toWords(this.amount);
//     }
//   },

//   methods: {
//     save() {
//       this.$store.dispatch("post", { path: this.path, body: this.refund });
//     },
//     update() {
//       this.$store.dispatch("put", { path: `${this.path}/${this.model.id}`, body: this.refund });
//     },
//     updateDialog() {
//       var obj = this.$store.state.search.selectedData[0].value;
//       this.setDialog(obj);
//     },
//     async search() {
//       var obj = this.$store.state.obj;
//       this.refund = Object.assign({}, obj);
//       this.setObjects(obj);
//     },
//     reset() {
//       this.model.clear();
//     },
//     setObjects(obj) {
//       console.log(obj);
//     }, setDialog(obj) {
//       this.refund = Object.assign({}, obj);
//       this.setObjects(obj);
//     },
//     done() {
//       this.$store.commit(
//         "crudtable/data",
//         Object.assign({}, this.refund)
//       );
//     },
//     updateCrudTableDialog() {
//       this.setDialog(this.$store.state.crudtable.data);
//     },
//     resetCrudTableDialog() {
//       this.reset();
//     },

//     appendIconCallback() {
//       let miniSelected = { path: receiptNav.menu.path, name: receiptNav.menu.name, headers: receiptNav.menu.miniHeaders };
//       this.$store.commit("search/miniSelected", miniSelected);
//       this.$store.dispatch("search/getMiniData");
//       this.$store.commit("search/miniDialog", true);

//     },

//     receiptIdEntered(){

//       this.$store.commit("sales/receipt/obj", null);

//       if (this.receiptId) {
//          console.log("Dispatched Receipt Id", this.receiptId)
//           this.$store.dispatch("sales/receipt/get", this.receiptId);
//       }
//     },

//     setControls(){

//       if(this.model.refundType ===this.cancellationRefundType){
//          let receipt = this.receipt;
//          this.model.creditNoteId = "";
//        this.creditNoteIdRules = [];
//          if(receipt){
//             this.model.bankAccountType = receipt.bankAccountType;
//             this.model.bankAccount = {id: receipt.bankAccountId};
//             this.model.amount = receipt.amountReceived;

//          }
//       }
//       else if(this.model.refundType ===this.balanceClaimRefundType){
//          let receipt = this.receipt;
//          this.model.creditNoteId = "";
//        this.creditNoteIdRules = [];
//          if(receipt){

//             this.model.amount = this.change;

//          }
//       }

//     },

//   }
// };
//
</script>
