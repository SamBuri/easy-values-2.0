...
const funcs = {
  ...
  calculateAmount(model) {
    if (model.amountTendered && model.exchangeRate)
      model.amount = model.amountTendered * model.exchangeRate;
    else model.amount = 0;
    model.amountWords = this.toWords(model.amount);
  },

  calculateAmountFromUnitPrice(model) {
    if (model.quantity && model.unitPrice)
      model.amount = model.quantity * model.unitPrice;
    else model.amount = 0;
  },

  calculateAmountFromUnitCost(model) {
    if (model.quantity && model.unitCost)
    ...
    if (toReplace) {
      if (
        toReplace
          .toString()
          .toLowerCase()
          .startsWith(replaceWith.toString().toLowerCase())
      ) {
        return replaceWith;
      }
    }
