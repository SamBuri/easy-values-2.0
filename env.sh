...
# Extract EV_APP_ environment variables and build the sed expression
env | grep '^EV_APP_' | while IFS='=' read -r key value; do
...
if [ ! -s "$sed_script" ]; then
    echo "No EV_APP_ environment variables found. Exiting with error."
    exit 1
fi
